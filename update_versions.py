...
def process_chart(
    ...
    chart_yaml_path = os.path.join(CHARTS_DIR, path, "Chart.yaml")
    ...
    if disabled:
        ...
        # Update chart info; a failed lookup keeps the cached version but is
        # still counted so the run exits non-zero.
        if new_ver:
...
def main() -> int:
    ...
    # Update README — skipped on single-chart runs, which would truncate the
    # table to that one chart's row.
    if args.chart:
    ...
    # A red exit surfaces lookup failures in cron runs; the successful
    # updates are already written above.
